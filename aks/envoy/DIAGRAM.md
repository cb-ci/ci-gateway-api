...
This diagram illustrates how external traffic reaches the CloudBees CI Operations Center (`cjoc`) and Managed Controllers (e.g., `ha`) when using **Envoy Gateway** as the ingress controller on AKS.
...
graph TD
    ...
    subgraph "Azure Cloud (LoadBalancer)"
        LB["AKS LoadBalancer Service<br/>(provisioned by Envoy Gateway)"]
    end
...
2. **AKS LoadBalancer**: A `Service` of type `LoadBalancer` automatically provisioned by Envoy Gateway. Exposes the Envoy proxy pods externally via Azure Load Balancer.
3. **GatewayClass (`eg`)**: Links the Gateway resource to the Envoy Gateway controller (`gateway.envoyproxy.io/gatewayclass-controller`).
...
5. **HTTPRoute (`cloudbees-route`)**: Path-based routing:
   - `/cjoc/*` → `cjoc` Service
   - `/ha/*` → `ha` Service
6. **BackendTrafficPolicy (cjoc)**: Configures active HTTP health checks probing `/cjoc/health/` on the `cjoc` Service.
7. **BackendTrafficPolicy (ha)**: Configures active HTTP health checks on `/ha/health/` **and** cookie-based consistent-hash load balancing (sticky sessions) via `CBCI_SESSION`.
8. **Envoy Proxy Pods**: The data-plane sidecar pods managed by Envoy Gateway. All routing, health checking, and session-affinity logic is enforced here — no Azure-specific configuration required.

## Key Differences vs. Azure Application Gateway

| Concern | Azure Application Gateway | Envoy Gateway |
| :--- | :--- | :--- |
| GatewayClass | Azure AGIC | `eg` |
| Load balancer | Azure Application Gateway | AKS Service `type: LoadBalancer` (Envoy pods) |
...
| TLS | Azure Key Vault or Kubernetes secret | Kubernetes TLS secret |
